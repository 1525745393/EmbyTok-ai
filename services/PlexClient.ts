...
export class PlexClient extends MediaClient {
  ...
  private async getMachineIdentifier(): Promise<string> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return (
          data.MediaContainer.machineIdentifier || data.MediaContainer.MachineIdentifier || '1'
        );
      }
    } catch (e) {}
    return '1';
  ...
  async authenticate(username: string, password: string): Promise<ServerConfig> {
    ...
    const machineIdentifier =
      data.MediaContainer.machineIdentifier || data.MediaContainer.MachineIdentifier;
    return {
      url: this.config.url,
  ...
  async getLibraries(): Promise<EmbyLibrary[]> {
    const response = await fetch(`${this.getCleanUrl()}/library/sections`, {
      headers: this.getHeaders(),
    });
    const data = await response.json();
  ...
  async getResumeItems(): Promise<EmbyItem[]> {
    try {
      const response = await fetch(`${this.getCleanUrl()}/library/onDeck`, {
        headers: this.getHeaders(),
      });
      const data = await response.json();
  ...
  async getVideos(
    ...
    if (feedType === 'favorites') {
      ...
      const response = await fetch(
        `${this.getCleanUrl()}/playlists/${playlist.ratingKey}/items?X-Plex-Container-Start=0&X-Plex-Container-Size=2000`,
        { headers: this.getHeaders() }
      );
      const data = await response.json();
      ...
      return {
        items: reversed.slice(skip, skip + limit),
    ...
    return items.map((p: any) => {
      ...
      return {
        ...
        UserData: {
          ...
          LastPlayedDate: p.lastViewedAt
            ? new Date(p.lastViewedAt * 1000).toISOString()
            : undefined,
        },
        _PlexThumb: p.thumb,
  ...
  getVideoUrl(item: EmbyItem, mode: 'direct' | 'transcode' | 'fallback' = 'direct'): string {
    ...
    const useTranscode = mode !== 'direct';
    if (!useTranscode && plexItem._PlexKey)
  ...
  private async findPlaylist(libraryName: string): Promise<any | null> {
    ...
    try {
      const response = await fetch(
        `${this.getCleanUrl()}/playlists?title=${encodeURIComponent(title)}`,
        { headers: this.getHeaders() }
      );
      const data = await response.json();
  ...
  async getFavorites(libraryName: string): Promise<Set<string>> {
    ...
    try {
      const response = await fetch(
        `${this.getCleanUrl()}/playlists/${playlist.ratingKey}/items?X-Plex-Container-Size=2000`,
        { headers: this.getHeaders() }
      );
      const data = await response.json();
  ...
  async toggleFavorite(itemId: string, isFavorite: boolean, libraryName: string): Promise<void> {
    ...
    if (isFavorite) {
      ...
      const entry = (await itemsRes.json()).MediaContainer.Metadata?.find(
        (i: any) => i.ratingKey === itemId
      );
      if (entry?.playlistItemID) {
        await fetch(
          `${this.getCleanUrl()}/playlists/${playlist.ratingKey}/items/${entry.playlistItemID}?X-Plex-Token=${this.config.token}`,
      ...
      if (playlist) {
        await fetch(
          `${this.getCleanUrl()}/playlists/${playlist.ratingKey}/items?uri=${encodeURIComponent(itemUri)}&X-Plex-Token=${this.config.token}`,
          { method: 'PUT', headers: this.getHeaders() }
        );
      } else {
        await fetch(
          `${this.getCleanUrl()}/playlists?type=video&title=${encodeURIComponent(`Tok-${libraryName}`)}&smart=0&uri=${encodeURIComponent(itemUri)}&X-Plex-Token=${this.config.token}`,
  ...
  async searchItems(query: string): Promise<EmbyItem[]> {
    try {
      const response = await fetch(
        `${this.getCleanUrl()}/search?query=${encodeURIComponent(query)}`,
        {
          headers: this.getHeaders(),
        }
      );
        ...
        metadata.Media.forEach((media: any) => {
          if (media.Part) {
            media.Part.forEach((part: any) => {
              if (part.Stream) {
                part.Stream.filter((stream: any) => stream.streamType === 3) // 3 = subtitle
                  .forEach((stream: any) => {
