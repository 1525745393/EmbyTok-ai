...
export const getTokPlaylistItems = async (
  serverUrl: string,
  userId: string,
  token: string,
  libraryName: string
  ...
  try {
    ...
    const response = await fetch(
      `${cleanUrl}/Playlists/${pid}/Items?UserId=${userId}&Fields=MediaSources,Width,Height,Overview,UserData`,
      {
        headers: getHeaders(token),
      }
    );
...
export const addToTokPlaylist = async (
  serverUrl: string,
  userId: string,
  token: string,
  libraryName: string,
...
export const removeFromTokPlaylist = async (
  serverUrl: string,
  userId: string,
  token: string,
  libraryName: string,
  ...
  const itemsRes = await fetch(
    `${cleanUrl}/Playlists/${pid}/Items?Fields=Id,PlaylistItemId&UserId=${userId}`,
    { headers: getHeaders(token) }
  );
